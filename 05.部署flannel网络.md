...
完整 unit 见 [flanneld.service](https://github.com/shean023/k8s/edit/master/systemd/flanneld.service)
...
`{"PublicIP":"192.168.52.132","BackendType":"vxlan","BackendData":{"VtepMAC":"aa:35:af:24:74:55"}}`
...
>>> 192.168.52.131
    inet 172.30.46.0/32 scope global flannel.1
>>> 192.168.52.132
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh ${node_ip} "ping -c 2 172.30.49.0"
    ssh ${node_ip} "ping -c 2 172.30.46.0"
    ssh ${node_ip} "ping -c 2 172.30.43.0"
  done
```
