...
本文档讲解部署一个 三实例 kube-apiserver 集群的步骤。
...
替换后的 encryption-config.yaml 文件：[encryption-config.yaml](https://github.com/shean023/k8s/tree/master/systemd/encryption-config.yaml)
...
替换后的 unit 文件：[kube-apiserver.service](https://github.com/shean023/k8s/tree/master/systemd/kube-apiserver.service)
...
注意：
1. 如果执行 kubectl 命令时输出如下错误信息，则说明使用的 `~/.kube/config` 文件不对，请切换到正确的账户后再执行该命令：
...
tcp        0      0 192.168.52.130:6443     0.0.0.0:*               LISTEN      51903/kube-apiserve
```
